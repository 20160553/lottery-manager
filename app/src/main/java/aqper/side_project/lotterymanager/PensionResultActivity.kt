...
class PensionResultActivity : AppCompatActivity(), CoroutineScope {
        ...
        launch {
            withContext(Dispatchers.IO) {
                try {
                    ...
                    var result =
                        doc.select("strong.result").text().toString().replace(" 당첨", "").split(" ")

                    if ((result.size > 0).not()) {
                        result = listOf("미추첨", "미추첨")
                    }
    ...
    private fun initLayout() {
        binding.registResultButton.setOnClickListener {
            myPensionResult ?: return@setOnClickListener
            var wAmmount = 0
            launch {
                withContext(Dispatchers.IO) {
                    DatabaseProvider.provideDB(this@PensionResultActivity).myPensionResultDao()
                        .insert(
                            MyPensionResultEntity(
                                null,
                                roundText,
                                myPensionResult!!
                            )
